...
def prepare():
    ...
    active = ROOT / "docs/reviews/issue43-lz4-runtime/profile-rebinding/catalog.json"
    ...
    manifest["goldens"] = []
    manifest_path = ROOT / "docs/reviews/issue43-lz4-runtime/unqualified-manifest.json"
    ...
    for token in candidate["tokens"]["fixture"]:
        ...
        if fixture["state"] != "available" or "corpora/eae93a2c9aca5b04/" not in fixture["provenance"]["manifest"]["path"]:
            continue
        old_token = token["token"]
        token["token"] += "-tracked-source-v2"
        ...
        if raw_binding["sha256"] != source_fixture["output"]["sha256"]:
    ...
    target = ROOT / "target/issue43-tracked-source-catalog.json"
    ...
    (ROOT / "docs/reviews/issue43-lz4-runtime/source-repair.json").write_bytes(canonical(receipt))
