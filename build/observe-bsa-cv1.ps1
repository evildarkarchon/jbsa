...
if ($phase -eq 'oracle-to-jbsa') {
    ...
    $inputArchive = Join-Path (Get-Location) 'input.bsa'
...
if ($operation -eq 'encode' -and $case.identity.codec -eq 'raw-deflate') {
    $stream = & $java --enable-native-access=ALL-UNNAMED -cp $classpath `
        io.github.evildarkarchon.jbsa.cli.Main pack $inputArchive candidate.bsa -tes4 -z:raw-deflate 2>&1
    ...
    $text = & $java --enable-native-access=ALL-UNNAMED -cp $classpath `
        io.github.evildarkarchon.jbsa.verification.BsaPublicObservation $inputArchive $operation $case.identity.codec
...
$archive = if (Test-Path -LiteralPath 'candidate.bsa') { 'candidate.bsa' } else { 'input.bsa' }
