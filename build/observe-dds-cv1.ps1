...
$recipe = $case.metadata.fixture_binding.generator.configuration.recipe
...
if ($operation -eq 'encode' -and $case.identity.codec -eq 'raw-deflate') {
    $stream = & $java --enable-native-access=ALL-UNNAMED -cp $classpath `
        io.github.evildarkarchon.jbsa.cli.Main pack $inputArchive candidate.ba2 -fo4dds -z:raw-deflate 2>&1
    $observed = @{ exit_status = $LASTEXITCODE; artifact_exists = (Test-Path -LiteralPath 'candidate.ba2') }
} elseif ($recipe -eq 'dds-cli-selector' -and $phase -ne 'oracle-to-jbsa') {
    $stream = & $java --enable-native-access=ALL-UNNAMED -cp $classpath `
        io.github.evildarkarchon.jbsa.cli.Main pack $inputArchive candidate.ba2 -fo4dds -split:0 -share:no -mt:no 2>&1
    if ($LASTEXITCODE -ne 0) { throw "DDS selector CLI failed: $stream" }
    $text = & $java --enable-native-access=ALL-UNNAMED -cp $classpath `
        io.github.evildarkarchon.jbsa.verification.DdsPublicObservation candidate.ba2 decode zlib pc-v1
    if ($LASTEXITCODE -ne 0) { throw 'DDS selector observation process failed' }
    $observed = ($text -join "`n") | ConvertFrom-Json -AsHashtable -Depth 100
} else {
    $text = & $java --enable-native-access=ALL-UNNAMED -cp $classpath `
        io.github.evildarkarchon.jbsa.verification.DdsPublicObservation $inputArchive $operation $case.identity.codec $case.identity.configuration $recipe
