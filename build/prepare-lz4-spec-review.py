...
OUTPUT = ROOT / "docs/reviews/issue43-lz4-runtime/specification-rebinding"
...
def prepare_catalog(catalog, specification, proposed):
    ...
    for proposal in proposed:
        ...
        records.append({"schema_version": 1, "golden_id": proposal["proposed_case_id"].lower().replace(".", "-"),
            ...
            "rationale": "LZ4 runtime specification 0.14.0 rebinding; original independently approved observations retained.",
    ...
    (OUTPUT / "baseline-catalog.json").write_bytes((ROOT / "tests/conformance/catalog.json").read_bytes())
    (OUTPUT / "pending-records.json").write_bytes(canonical(records))


def validate_successors(baseline, specification, proposals):
    """Reject lost cases, changed observations, provider drift or unbound successors.

    Compare the complete proposal with admitted evidence without executing product
    code or treating newly generated output as expected observations.
    """
    candidate = json.loads((OUTPUT / "catalog.json").read_bytes())
    if len(proposals) != 109 or len(candidate["cases"]) != len(baseline["cases"]):
        raise ValueError("Rebinding must retain all cases and exactly 109 admitted goldens")
    for key in baseline:
        if key not in {"cases", "tokens", "specification_set"} and baseline[key] != candidate[key]:
            raise ValueError(f"Unrelated catalog field changed: {key}")
    for key in baseline["tokens"]:
        if key != "fixture" and baseline["tokens"][key] != candidate["tokens"][key]:
            raise ValueError(f"Unrelated token mapping changed: {key}")
    for old_case, new_case in zip(baseline["cases"], candidate["cases"]):
        for key in old_case["identity"]:
            if key not in {"case_id", "fixture"} and old_case["identity"][key] != new_case["identity"][key]:
                raise ValueError(f"Runtime/provider case identity changed: {key}")
        old_files = old_case["metadata"]["fixture_binding"]["files"]
        new_files = new_case["metadata"]["fixture_binding"]["files"]
        if [f["sha256"] for f in old_files] != [f["sha256"] for f in new_files]:
            raise ValueError("Fixture bytes changed")
    expected_digest = hashlib.sha256(canonical(specification)).hexdigest()
    for proposal in proposals:
        old = json.loads((ROOT / proposal["old"]["path"]).read_bytes())
        new = json.loads((ROOT / proposal["proposed"]["path"]).read_bytes())
        if new["specification_sha256"] != expected_digest:
            raise ValueError("Successor lacks current specification identity")
        if new["case_id"] != proposal["proposed_case_id"]:
            raise ValueError("Successor case identity differs from proposal")
        for key in old:
            if key not in {"case_id", "specification_sha256"} and old[key] != new[key]:
                raise ValueError(f"Golden observation changed: {key}")
...
def prepare():
    ...
    for path, entry in sorted(goldens.items()):
        ...
        successor_fixture = identity["fixture"] + "-runtime-v1"
    ...
    validate_successors(catalog, specification, proposed)
    packet = {"status": "UNTRUSTED_PENDING_MAINTAINER_APPROVAL", "approval": None,
        ...
        "activation_requirements": ["Review governing specification changes, unchanged assertions and complete successor catalog.",
            "Approve this exact review packet digest and invoke the pinned activation implementation configured for this packet with the maintainer identity.",
