<# .SYNOPSIS Runs all proposed DDS BA2 successor cases as untrusted evidence pending maintainer review. #>
[CmdletBinding()]
...
$proposal = Join-Path $root 'docs/reviews/issue40-cv1/catalog.json'
$catalog = Read-ConformanceCatalog -Path $proposal -RepositoryRoot $root
$cases = @($catalog.cases | Where-Object {
    $_.identity.archive_family -eq 'fo4-dx10-v1' -and $_.identity.fixture -like 'fo4-dds-*'
})
if ($cases.Count -ne 44) { throw 'The scoped proposal must contain all 44 PC DDS BA2 cells.' }
$oracleInputs = Join-Path $root 'target/dds-cv1-review-inputs'
[IO.Directory]::CreateDirectory($oracleInputs) | Out-Null
foreach ($mode in @('base', 'single')) {
    [IO.File]::WriteAllBytes((Join-Path $oracleInputs "$mode.ba2"),
        [Convert]::FromHexString([IO.File]::ReadAllText((Join-Path $root "docs/reviews/issue40-cv1/oracle/$mode.hex")).Trim()))
    $boundWork = Join-Path $root "target/dds-cv1-oracle-$mode"
    [IO.Directory]::CreateDirectory($boundWork) | Out-Null
    Copy-Item -LiteralPath (Join-Path $oracleInputs "$mode.ba2") -Destination (Join-Path $boundWork "oracle.ba2") -Force
}
...
$adapter = Join-Path $PSScriptRoot 'observe-dds-cv1.ps1'
$scanner = Join-Path $PSScriptRoot 'dds-cv1-expectations.py'
...
$inputs = @($adapter, $scanner, $ddsValidator, $texdiag, (Join-Path $PSScriptRoot 'validate-dds-wire.py'),
    ...
    @(Get-ChildItem -LiteralPath (Join-Path $root 'jbsa-conformance-tests/target/test-classes/io/github/evildarkarchon/jbsa/verification') -Filter 'DdsPublicObservation*.class' | ForEach-Object FullName)
...
foreach ($case in $cases) {
    ...
    if ($case.metadata.expected_behavior -eq 'accept') {
        ...
        $selection = $case.metadata.fixture_binding.generator.configuration.recipe -eq 'dds-reconstruction-selection'
        ...
        $registration.validators = @(@{ input_role = 'archive'; assertion_id = $assertion;
            tool = @{ identity = 'jbsa-independent-specification-dds-ba2-scanner'; path = $python;
                sha256 = Get-ConformanceFileDigest $python; adapter_version = '1'; kind = 'archive';
                independent = $true; derived_from_reference = $false };
            arguments = $scannerArguments }, @{ input_role = 'archive'; assertion_id = $assertion;
