...
final class Ba2LocalCorpusIT {
  /**
   * Compares every available FO4 or Starfield General entry with pinned-oracle extraction without
  ...
  void readsOptionalLocalGeneralArchivesAgainstOracle() throws Exception {
    ...
    Map<Path, ArchiveFamily> corpora =
        Map.of(
            root.resolve("tests/fixtures/local/corpus/archives/fo4-gnrl/v1"),
                ArchiveFamily.FO4_GENERAL_BA2,
            root.resolve("tests/fixtures/local/corpus/archives/sf-gnrl/v2"),
                ArchiveFamily.STARFIELD_GENERAL_BA2,
            root.resolve("tests/fixtures/local/corpus/archives/sf-gnrl/v3-m3"),
                ArchiveFamily.STARFIELD_GENERAL_BA2);
    List<Path> available = corpora.keySet().stream().filter(Files::isDirectory).sorted().toList();
    org.junit.jupiter.api.Assumptions.assumeFalse(
        available.isEmpty(), "UNAVAILABLE: local General BA2 corpus");
      ...
      try (var paths = Files.list(corpus)) {
        ...
        for (Path source : archives) {
          ...
          try (OpenArchive archive =
            ...
            assertEquals(corpora.get(corpus), archive.inspection().metadata().family());
            ...
            for (long ordinal = 0; ordinal < archive.entryCount(); ordinal++) {
              ...
              try (var input = Files.newInputStream(expected)) {
                ...
                for (int count; (count = input.read(window)) >= 0; )
                  oracle.update(window, 0, count);
              }
              assertArrayEquals(
