...
try {
    ...
    Assert-CatalogRejected 'base-matrix membership cannot be false' { param($c) $c.cases[0].metadata | Add-Member base_matrix_cell $false -Force }
    Assert-CatalogRejected 'base-matrix membership cannot be a string' { param($c) $c.cases[0].metadata | Add-Member base_matrix_cell 'true' -Force }
